...
class YoloYoloModel(nn.Module):
    """Combined model for Czech railway light detection and classification"""

    def __init__(self, detection_net_path="/Users/danielschnurpfeil/PycharmProjects/czech-railway-trafic-lights-detection/classification_experiments/czech_railway_light_detection_backbone/detection_backbone/weights/best.pt",
        ...
        self.classification_head = YOLO("/Users/danielschnurpfeil/PycharmProjects/czech-railway-trafic-lights-detection/classification_experiments/yolo_training/czech_railway_lights/weights/best.pt")
    ...
    def forward(self, image, conf=0.5, iou=0.5, verbose=False):
        ...
        for detection in detections:
            ...
            for box in boxes:
                ...
                with torch.no_grad():
                    output = self.classification_head(crop)

                # Get prediction
                class_id = output[0].probs.top1
    ...
    def forward(self, image, conf=0.5, iou=0.5, verbose=False):
        ...
        for detection in detections:
            ...
            for box in boxes:
                ...
                if crop.size == 0:
                    continue
                # todo HUGE INCOMPATIBILITY ... the Python Imaging Library is trash
                crop_pil = Image.fromarray(crop)
                # Transform and classify
                image_tensor = self.transform(crop_pil).unsqueeze(0)  # Add batch dimension
                image_tensor = image_tensor.to(self.device)
...
if __name__ == "__main__":
    # Initialize model
    model = CzechRailwayLightModel(detection_net_path="/Users/danielschnurpfeil/PycharmProjects/czech-railway-trafic-lights-detection/classification_experiments/czech_railway_light_detection_backbone/detection_backbone/weights/best.pt",
)

...
parser.add_argument('--sequences_jsom_path', default="../traffic_lights_raw.json")
...
parser.add_argument('--out-dir', default="../reconstructed/all_yolov5mu_raw")
...
with open(args.sequences_jsom_path, encoding="utf-8", mode="r") as f:
    traffic_lights_raw = dict(json.load(f))
...
for i in traffic_lights:
    ...
    for j in traffic_lights_raw[video_name]:
        if d_video is not None:
            ...
            if detected == 0:
                ...
                if detected == 0:
                    ...
                    detected_count += detected
    cap.release()
    summary[d_video] = {"original": len(traffic_lights_raw[video_name]),
                        "detected": detected_count,
                        "lost_pictures": failed_pictures.copy(),
                        "lost_pictures_count": int(len(traffic_lights_raw[video_name]) - detected_count),
    ...
    print("------------------------------------------------------")
    detected_count = 0
with open(SAVE_PATH + "/summary.json", mode="w") as f:
